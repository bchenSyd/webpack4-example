...
ExternalsPlugin.prototype.apply = function(compiler) {
  ...
  compiler.hooks.normalModuleFactory.tap("my NMF plugin", __builtIn_nmf => {
    ...
    __builtIn_nmf.hooks.factory.tap(
      "my NMF factory plugin",
      (
        _buildin_factory_function
      ) /* default factory_functin provided by webpack */ => {
        ...
        const customerizedFactoryFunction = (__preResolve_result, callback) => {
          // as you can see
          _buildin_factory_function(__preResolve_result, (
            err,
            module /* the normal module that native nmf resolved */
          ) => {
            if (err) {
              return callback(err);
            }
